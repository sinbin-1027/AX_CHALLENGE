...
async function insertRawPurchases() {
  const data = JSON.parse(fs.readFileSync('./raw_purchases_full_batch.json', 'utf-8'));
  ...
  try {
    ...
    for (const row of data) {
      ...
      try {
        ...
        `, [
          ...
          row.회계연도, row.출납예정일자, row.통제일자, row.예산코드,
    ...
    console.log('\n=== 완료 ===');
    console.log(`✅ 신규 인서트: ${inserted}건 (이전에 제외됐던 키워드 행들 포함)`);
    console.log(`⏭ 이미 존재/스킵: ${skipped}건`);
